# Security Policy for `mastodon-finder`
...
COPY --chown=appuser:appuser mastodon_finder/ ./mastodon_finder/

# Set the entrypoint to run the tool
ENTRYPOINT ["mastodon-finder"]
CMD ["--help"]
```
...
```bash
# Build the image
docker build -t mastodon-finder .
...
docker run --rm -v "$(pwd):/data" mastodon-finder /data
```
